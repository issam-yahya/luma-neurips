#!/bin/bash
# =============================================================================
# LUMA / LUMA  --  reproduction driver
# =============================================================================
# Reproduces every reported best result by re-running run.py with the exact
# hyperparameters recorded in the original logs (reproduce/configs_luma_best.csv).
#
# Usage:
#   bash reproduce/reproduce_luma.sh                 # all datasets
#   bash reproduce/reproduce_luma.sh ETTh1 ETTh2     # only these datasets
#   GPU=1 bash reproduce/reproduce_luma.sh           # pick CUDA device
#   SEED=2021 bash reproduce/reproduce_luma.sh       # override random seed
#
# Outputs:
...
CONFIG="${SCRIPT_DIR}/configs_luma_best.csv"
...
# optional dataset filter from CLI args
FILTER=" $* "

echo "dataset,pred_len,P,R,mse,mae,params" > "${RESULTS}"

# --- iterate configs (skip CSV header) ---
tail -n +2 "${CONFIG}" | while IFS=, read -r dataset pred_len period_len rank seq_len \
    ...
    # apply dataset filter if any args were passed
